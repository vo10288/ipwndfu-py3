    ...
    for opt, arg in opts:
        ...
        if opt == '-x':
            ...
            for byte in nor_data.parts[1]:
                if byte != 0 if isinstance(byte, int) else byte != b'\x00':
        ...
        if opt == '--dump':
            ...
            if 'PWND:[checkm8]' in serial_number:
                ...
                sys.stdout.buffer.write(device.read_memory(address, length))
            else:
                device = PwnedDFUDevice()
        ...
        if opt == '--dump-rom':
            ...
            if 'PWND:[checkm8]' in serial_number:
                ...
                # Handle bytes conversion for Python 3
                if isinstance(securerom, bytes):
                    chip = securerom[0x200:0x240].decode('utf-8', errors='ignore').split(' ')[2][:-1]
                    kind = securerom[0x240:0x280].decode('utf-8', errors='ignore').split('\0')[0]
                    version = securerom[0x280:0x2C0].decode('utf-8', errors='ignore').split('\0')[0][6:]
                else:
        ...
        if opt == '--dump-nor':
            ...
            if device.config.cpid != '8920':
                ...
                sys.exit(1)
            nor_dump = device.nor_dump(saveBackup=False)
        ...
        if opt == '--flash-nor':
            ...
            # Check for IMG2 header magic in bytes
            if new_nor[:4] != b'IMG2'[::-1]:
        ...
        if opt == '--decrypt-gid':
            ...
            if 'PWND:[checkm8]' in serial_number:
                ...
                print(pwned.aes(bytes.fromhex(arg), usbexec.AES_DECRYPT, usbexec.AES_GID_KEY).hex())
            else:
                device = PwnedDFUDevice()
        ...
        if opt == '--encrypt-gid':
            ...
            if 'PWND:[checkm8]' in serial_number:
                ...
                print(pwned.aes(bytes.fromhex(arg), usbexec.AES_ENCRYPT, usbexec.AES_GID_KEY).hex())
            else:
                device = PwnedDFUDevice()
        ...
        if opt == '--decrypt-uid':
            ...
            if 'PWND:[checkm8]' in serial_number:
                ...
                print(pwned.aes(bytes.fromhex(arg), usbexec.AES_DECRYPT, usbexec.AES_UID_KEY).hex())
            else:
                device = PwnedDFUDevice()
        ...
        if opt == '--encrypt-uid':
            ...
            if 'PWND:[checkm8]' in serial_number:
                ...
                print(pwned.aes(bytes.fromhex(arg), usbexec.AES_ENCRYPT, usbexec.AES_UID_KEY).hex())
            else:
                device = PwnedDFUDevice()
        ...
        if opt == '--boot':
            ...
            if 'CPID:8015' not in serial_number or 'PWND:[checkm8]' not in serial_number:
                ...
                with open('bin/t8015_shellcode_arm64.bin', 'rb') as f:
                    shellcode = f.read()
