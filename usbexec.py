...
class ExecConfig:
    ...
    def match(self, info):
        ...
        target = self.info[0].ljust(0x40, '\0') + self.info[1].ljust(0x40, '\0') + self.info[2].ljust(0x80, '\0')
...
class PwnedUSBDevice():
    ...
    def command(self, request_data, response_length):
        ...
        if response_length == 0:
            response_data = device.ctrl_transfer(0xA1, 2, 0xFFFF, 0, response_length + 1, CMD_TIMEOUT)
            if hasattr(response_data, 'tobytes'):
                response = response_data.tobytes()[1:]
            else:
                response = bytes(response_data)[1:]
        else:
            response_data = device.ctrl_transfer(0xA1, 2, 0xFFFF, 0, response_length, CMD_TIMEOUT)
            if hasattr(response_data, 'tobytes'):
                response = response_data.tobytes()
            else:
    ...
    def execute(self, response_length, *args):
        ...
        for i in range(len(args)):
            ...
            elif isinstance(args[i], (str, bytes)) and i == len(args) - 1:
                if isinstance(args[i], str):
                    cmd += args[i].encode('latin-1')
                else:
                    cmd += args[i]
            else:
